...
while getopts 'd:g:s:c:a:h' flag; do
  case "${flag}" in
...
if [[ -z "$SERVICE_CATEGORY" ]]; then
    SERVICE_CATEGORY=$(echo $PROJECT_NAME | sed 's/-java-sdk//')
fi
...
if [[ -z "$PROJECT_DESCRIPTION" ]]; then
    PROJECT_DESCRIPTION="Java SDK for IBM Cloud ${SDK_NAME} services"
fi
...
printf ">>>>> build.yaml and publish.yaml updated.\n"
...
sed -i.bak "s/github.ibm.com/github.com/g" README.md
